...
export const Screen = ({ children, className = '' }: { children?: React.ReactNode, className?: string }) => {
  return (
    <div className={`flex flex-col h-full w-full lg:max-w-md lg:mx-auto bg-wechat-bg relative lg:shadow-2xl overflow-hidden ${className}`}>
      {children}
    </div>
  );
};
export const Header = ({ title, onBack, rightAction, dark = false }:
