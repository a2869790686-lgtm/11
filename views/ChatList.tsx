...
export const ChatList = ({ onNavigate }: { onNavigate: (view: ViewState) => void }) => {
  const { getChatSessions, stickyChatIds, t } = useStore();
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <ScrollArea className="bg-white">
        {sessions.map(session => {
          const isSticky = stickyChatIds.includes(session.id);
          return (
            <div 
              key={session.id}
            ...
            >
              ...
              <div className="ml-3 flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-0.5">
                  <h3 
                    className="text-[16px] font-medium text-black truncate" 
                    onClick={(e) => {
                        e.stopPropagation();
                        onNavigate({ type: 'USER_PROFILE', userId: session.id });
                    }}
                ...
                <p className="text-sm text-gray-400 truncate leading-tight">
                    {session.lastMessage?.content || '你可以开始聊天了'}
                </p>
              </div>
            </div>
          );
        })}
        <div className="py-12 text-center text-gray-300 text-xs tracking-widest bg-white font-light">
          已加载全部消息
        </div>
      </ScrollArea>
    </div>
